...
export function isServerMeterAction(a: ServerType<EditableAction<ActionType>>): a is ServerType<EditableAction<"meter">> {
    return a.type === "meter";
}
...
export type ActionRenderData<T extends ActionType> = {
    ...
    zone: string,
    location: string,
} & (T extends "vote" ? { voteOptions: VoteOption[] } : T extends 'meter' ? { meterType: "fun" | "budget", value: number } : { filePath: string });
